...
cedar: dist/cedar/pixman-0.32.6-1.tar.gz dist/cedar/freetype-2.5.5-1.tar.gz dist/cedar/giflib-4.2.3-1.tar.gz dist/cedar/pango-1.36.8-1.tar.gz dist/cedar/cairo-1.14.2-1.tar.gz dist/cedar/fontconfig-2.11.93-1.tar.gz dist/cedar/harfbuzz-0.9.39-1.tar.gz
...
dist/cedar/fontconfig-2.11.93-1.tar.gz: cairo-cedar
	docker cp $<:/tmp/fontconfig-cedar.tar.gz .
	mkdir -p $$(dirname $@)
	mv fontconfig-cedar.tar.gz $@
...
dist/cedar/harfbuzz-0.9.39-1.tar.gz: cairo-cedar
	docker cp $<:/tmp/harfbuzz-cedar.tar.gz .
	mkdir -p $$(dirname $@)
	mv harfbuzz-cedar.tar.gz $@

dist/cedar/pango-1.36.8-1.tar.gz: cairo-cedar
	docker cp $<:/tmp/pango-cedar.tar.gz .
	mkdir -p $$(dirname $@)
	mv pango-cedar.tar.gz $@
...
cairo-cedar: cedar-stack cairo-cedar/pixman.tar.gz cairo-cedar/freetype.tar.bz2 cairo-cedar/giflib.tar.bz2 cairo-cedar/cairo.tar.xz cairo-cedar/pango.tar.xz cairo-cedar/fontconfig.tar.bz2 cairo-cedar/harfbuzz.tar.bz2
	docker build --rm -t mojodna/$@ $@
	-docker rm $@
...
cairo-cedar/fontconfig.tar.bz2: src/fontconfig.tar.bz2
	ln -f $< $@
...
cairo-cedar/harfbuzz.tar.bz2: src/harfbuzz.tar.bz2
	ln -f $< $@

cairo-cedar/pango.tar.xz: src/pango.tar.xz
	ln -f $< $@
